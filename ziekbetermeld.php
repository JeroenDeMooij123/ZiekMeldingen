...
<?php
	...
	if(isset($_POST['btnOpslaan']))
	{
		...
		*/
		...
		$query = "INSERT INTO tijdsduur (naam, bdatum, edatum) VALUES "."('$naam', '$bdatum', '$edatum')";
		$stm = $con->prepare($query);
        if($stm->execute())
		$query = "UPDATE leerlingen SET naam = '$naam', klas = '$klas', bdatum = '$bdatum', edatum = '$edatum', status = '$status', details = '$details' WHERE ID = $ID";
		$stm = $con->prepare($query);
        if($stm->execute())
        
		{
			header('Location: /Ziekmeldingen/Ziekmelden.php' 	);
		}
	}
	...
	if($stm->execute())
	{
        ...
        <label>Begindatum</label></br>   <input type="text" name="txtbdatum"  readonly value="<?php echo $res->bdatum; ?>"/></br>
        <label>Einddatum</label></br>   <input type="text" name="txtedatum" value="<?php echo $res->edatum; ?>"/></br>
		<label>status</label></br>	<input type="text" name="txtStatus"readonly value="beter"/></br>
		<label>details</label></br>	<input type="text" name="txtdetails" readonly value="<?php echo $res->details; ?>"/></br>
